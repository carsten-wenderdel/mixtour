...
final class Node {
    ...
    func simulate3<T>(moveBuffer: inout MIXMoveArray, rng: inout T) -> MIXCorePlayer where T: RandomNumberGenerator {
        var board = state
        var iterations = 100 // to prevent infinite loops we stop after 100 moves
        while (!Core.isGameOver(&board) && iterations != 0) {
            iterations -= 1
            Core.arrayOfLegalMoves(&board, &moveBuffer)
            let randomIndex = Int.random(in: 0..<moveBuffer.n, using: &rng)
            var randomMove = moveBuffer.a[randomIndex]

            if moveBuffer.n > 1 {
                var otherIndex = randomIndex
                while (otherIndex == randomIndex) {
                    otherIndex = Int.random(in: 0..<moveBuffer.n, using: &rng)
                }
                let otherMove = moveBuffer.a[otherIndex]
                var tryBoard = board
                Core.makeMove(&tryBoard, move!)
                Core.arrayOfLegalMoves(&tryBoard, &moveBuffer)
                if moveBuffer.n == 1 {
                    let winner = Core.potentialWinner(&tryBoard, moveBuffer.a[0])
                    if (winner == Core.playerOnTurn(&board)) {
                        randomMove = otherMove
                    }
                }
            }
            Core.makeMove(&board, randomMove)
    ...
    func simulate2<T>(moveBuffer: inout MIXMoveArray, rng: inout T) -> MIXCorePlayer where T: RandomNumberGenerator {
        var board = state
        var iterations = 100 // to prevent infinite loops we stop after 100 moves
        while (!Core.isGameOver(&board) && iterations != 0) {
            iterations -= 1

            var moves = Board.allLegalMoves(&board).shuffled(using: &rng)

            let randomMove = moves.first!

            var move: MIXCoreMove?
            var foundMove = false
            if (moves.count == 1) {
                move = moves.popLast()
            } else {
                while (!moves.isEmpty && !foundMove) {
                    move = moves.popLast()
                    var tryBoard = board
                    Core.makeMove(&tryBoard, move!)
                    Core.arrayOfLegalMoves(&tryBoard, &moveBuffer)
            ...
            let theMove = foundMove ? move: randomMove
            Core.makeMove(&board, theMove!)
        }
        return Core.winner(&board) // will return a draw if game is not finished.
    }
